<!-- 工作稿：正式仿真及双教师盲审完成前不得作为投稿版本。 -->

# 状态驱动的三阶段程序设计引导：方法框架、仿真评估与课堂过程证据

## 摘要

> 待正式仿真、自动评审与双教师盲审完成后重写。当前段落仅保留课堂证据，不提前填入仿真方向。
...
另一些研究把智能体用于学习者模拟。Agent4Edu利用画像、记忆和行动模块生成学习者响应[4]，Classroom Simulacra依据学生历史与课程材料模拟在线学习行为[5]。Yusuf等对92篇高等教育对话智能体研究的综述，则从教学目的、角色和交互方式界定智能体[6]。课堂系统面对真实学生，只保存会话内活动状态，不建立跨作业画像。为比较不同引导条件，本文另行使用冻结的虚拟学生生成仿真轨迹，并把仿真机制证据与课堂行为证据分开报告。
...
本文把三个阶段概括为“表达—辨识—讲解与修正”的活动链。“活动链”是对学习方法的归纳，不是新理论；平台只是实施和记录这套方法的载体。研究分四个问题展开：

RQ1：完整三阶段框架与直接回答、固定顺序三阶段条件在机制指标和教师盲评上有何差异？

RQ2：移除状态适应、虚拟学生讲解或错误代码修正后，仿真交互呈现哪些变化？

RQ3：学生在真实课程中如何采用三阶段引导，会话沿哪些路径推进或中止？

RQ4：首次提交前的引导状态与提交表现如何共变？

RQ1和RQ2使用冻结的虚拟学生仿真，评价机制表现与交互质量；RQ3和RQ4使用匿名课堂日志，描述采用、过程和结果关联。两部分都不检验真实学生的学习增益。
...
本文把方法机制和课堂应用分开考察。RQ1比较完整三阶段框架、直接回答助手和固定顺序三阶段助手，考察三者在引导质量、认知激活和答案克制上的差异。RQ2通过三种消融条件检查状态适应、虚拟学生讲解和错误代码修正分别承担什么作用。RQ3转向真实课堂，描述学生是否进入、如何推进、在哪些环节退出。RQ4考察首次提交前的引导暴露与提交表现如何共变。前两个问题使用虚拟学生仿真，只讨论机制和交互质量；后两个问题使用匿名课堂日志，不作学习效果的因果解释。
...
RQ3使用全期492次会话统计采用情况，并用稳定版398次会话分析阶段路径。采用指标包括使用学生、完成会话、重复用户、跨作业用户及用户集中度；重复用户指会话数不少于2，跨作业用户指在不少于2项作业上产生会话。稳定版路径分类按最远有效阶段和完成状态确定。

稳定版398次会话的路径分为：没有形成第一阶段有效记录、第二阶段未完成、第三阶段未完成、全部完成。活跃时长按相邻事件时间差累加，每个间隔最多计5分钟，降低离开页面造成的长时间空档。阶段摩擦报告中位数和四分位数。事件转换先按时间排序，把日志映射为提交、提示、验证、对话、修正和通过等类别，再折叠连续重复类别；主图只显示完成组与未完成组合计不少于10次且覆盖不少于5个会话的转换。

RQ4以学生—作业为单位。对每一对记录该作业第一次代码提交及最终提交。在第一次提交前没有引导会话记为“无暴露”；已经开始但尚未完成记为“未完成暴露”；已有状态为完成且完成时间不晚于第一次提交记为“完成暴露”。结果包括首次满分、首次沙箱通过率、最终满分、最终通过率和提交次数。
...
若完整框架在答案克制、认知激活或适应性上高于直接回答与固定顺序基线，本文只把这一结果解释为冻结仿真条件下的交互差异。消融比较与教师盲审指向一致时，再讨论相应机制的作用；单一自动指标或个别对话不足以支持框架完整性的结论。
...
状态驱动调节与固定提示的差别，在于后续内容是否读取本次会话已经发生的动作。固定提示按预设顺序推进；完整框架结合回答、提示次数、验证结果和对话历史选择局部提示或追问。两者的差异由冻结仿真实验比较，结果只用于描述机制表现和交互质量。

### 5.3 会话内智能体与学习者模拟的边界

Agent4Edu和Classroom Simulacra保存画像、历史或反思状态，用于生成学习者响应并评价模拟质量[4-5]。课堂部署中的教师型Agent和虚拟学生面对真实学生，只读取单次会话状态，用来推进三阶段活动；跨作业使用仅表示同一学生在多项作业中建立过会话，不表示智能体具有跨作业记忆。仿真实验中的虚拟学生则是独立的评价角色，只生成合成回应，不参与真实课堂，也不用于替代真实学习者数据。
