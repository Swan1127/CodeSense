...
>
> 链接约定：a) 结构/目录与普通文件引用一律使用**仓库相对链接**，在本仓库任意分支/PR 上均可解析；b) 依托【源码确认】基线的机制结论一律使用指向上述基线完整 SHA 的 GitHub 源码链接（`https://github.com/XiaoCow666/CodeSense/blob/22f93d4/<路径>`），并尽量锚定到函数或行号，便于读者独立核对。
>
...
**A. 网页表单路径（异步，主流）**：`POST /submit/<assignment_id>`（[routes/assignments.py](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/assignments.py)）创建 `Submission(status=pending)` → 投递后台线程 [tasks/submission_tasks.py](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/tasks/submission_tasks.py)::`evaluate_submission_async` → 页面跳「评测中」，前端轮询/SSE 进度。后台按序执行：
...
**B. API 路径（同步）**：`POST /api/submit`（[routes/api.py](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/api.py)）同步评测并直接返回 JSON。
...
- **阶段三（费曼双 Agent）**：`forum/message` → `utils/agents/orchestrator.py` 意图路由 + 学生/教师角色仲裁 + AgentLoop 多轮 + coverage 掌握度判定 + SSE 流式返回；`forum/trace` 轨迹复盘。阶段完成状态的写入与收口分两层（基线代码中**不存在** `/server-verify` HTTP 路由，此处据源码修正早期表述）：
  1. **写入——仅由 runtime 内的 `_complete_session` 完成**（[utils/agents/feynman.py `_complete_session`](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/utils/agents/feynman.py#L1248-L1270)）：负责写入 `stage_pass`（`metadata.validated=True`，若会话尚无）并把会话置 `stage3_completed=True / status=completed`；它只在服务端校验通过时被触发，共两条触发源——① [`evaluate_fix`](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/utils/agents/feynman.py#L1010-L1026)（对应 `POST /thinking/api/stage3/fix_code`，见 [routes/thinking.py](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/thinking.py#L1901-L1906)）判定修复 `correct=True`（`source="validated_evaluation"`）；② 会话目标经服务端确认达成，[`_sync_completed_goal`](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/utils/agents/feynman.py#L1226) 以 `source="complete_goal"` 触发。
  2. **收口——只复核、不写入**：`POST /thinking/api/complete_session`（[routes/thinking.py](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/thinking.py#L1931-L1933)）先经 [`_stage3_completion_is_verified()`](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/thinking.py#L264-L281) 复核会话内已存在 validated 的 stage3 `stage_pass`，通过后才执行正式收口并记录演示提交；否则返回 409（`STAGE3_COMPLETION_NOT_VERIFIED`）。
...
### 8. SSE 协议与前端（源码确认）
统一 SSE 协议 `type: start/delta/status/done(error)/error`，前端统一入口 [static/js/sse-client.js](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/static/js/sse-client.js)（`consumeSSE`）。代码编辑器降级链：**Monaco（多 CDN 源轮询）→ CodeMirror 5.65.2 → textarea**，编辑器内容实时回写 textarea。
...
7. **CSRF：token 输出 ≠ 服务端校验；校验点仅 9 处（中危，源码确认＋范围限定）**：基线**未注册全局 `CSRFProtect`**（全仓 grep 无该扩展）。服务端校验只发生在处理器对 **FlaskForm 调用 `validate_on_submit()`/`validate()`** 时（Flask-WTF 在 `validate()` 阶段校验 token；`WTF_CSRF_ENABLED` dev/prod 未显式配置、沿用默认 True，仅 [config.py](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/config.py#L130-L131) testing 关闭）。基线中的**校验调用点全量 9 处**：assignments.py [L312](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/assignments.py#L312)、[L514](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/assignments.py#L514)、[L1145](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/assignments.py#L1145)、[L1209](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/assignments.py#L1209)，auth.py [L66](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/auth.py#L66)、[L164](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/auth.py#L164)、[L259](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/auth.py#L259)，users.py [L283](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/users.py#L283)、[L337](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/routes/users.py#L337)；受其保护的表单类为 [forms.py](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/forms.py#L10-L61) 中 6 个 `FlaskForm`（Login/Registration/Assignment/Submission/EditProfile/ChangePassword）。**token 输出站点共 9 个模板**，含 `form.hidden_tag()`（[add_assignment.html L110](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/templates/add_assignment.html#L110)、[edit_assignment.html L19](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/templates/edit_assignment.html#L19)、[login.html L586](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/templates/login.html#L586)、[register.html L262](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/templates/register.html#L262)、[register_teacher.html L83](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/templates/register_teacher.html#L83)、[submit_code.html L811](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/templates/submit_code.html#L811)、[teacher_add_assignment.html L42](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/templates/teacher_add_assignment.html#L42)）与显式 `csrf_token`（[change_password.html L13](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/templates/change_password.html#L13)、[edit_profile.html L96](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/templates/edit_profile.html#L96)）。机制含义：a) 在这些校验点，**缺失/错误 token 会让 `validate_on_submit()` 返回 False 而被拒**——不是“不校验”；b) **给模板补 token 字段只让该表单能通过其自身校验，不能为未走 WTForm 的路由增加任何保护**；c) 未命中上述 9 个校验点的 POST 处理器（自定义表单与 fetch/JSON 接口）确实**没有服务端 CSRF 校验**——此结论由“无 CSRFProtect + 未调用表单校验”这一机制推出，其中 A.1 的 batch 端点已单独核实（不实例化任何 WTForm 亦不调用校验，源码确认）；但「**全部** fetch/JSON 一律无校验」的穷举式表述未逐处理器附清单，超出部分按**待核实**对待。缓解：`SESSION_COOKIE_SAMESITE='Lax'`（[config.py L155](https://github.com/XiaoCow666/CodeSense/blob/22f93d4/config.py#L155)）挡跨站 POST 携带 Cookie，但对同站子域、顶层 GET 导航不设防。
...
- **设计亮点**：演示库请求级动态绑定 + 线程级二次校验的隔离设计，与「后台任务异步评测」配合，是单体应用里少见的干净实践；三阶段 + 费曼双 Agent 的完成态由服务端权威校验收口——`validated=True` 的 `stage_pass` 只在服务端判定通过（修复正确或目标达成，见三.4 写入路径）时由 runtime 写入，`complete_session` 接口仅复核该证据后置完成态，因此完成状态可信。
