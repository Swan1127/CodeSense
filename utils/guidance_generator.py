...
def generate_guidance(code, assignment_title, assignment_description, language="cpp"):
    ...
    if initialized and guidance_generator:
        try:
            prompt = f"""
作为一名经验丰富的编程导师，请分析以下学生编写的{language.upper()}代码，并提供循序渐进的指导建议。
请注意：你的目标是引导学生思考并自己解决问题，而不是直接提供完整答案。
...
请提供以下格式的指导建议：
1. 简要分析当前代码的完成情况和存在的问题
2. 提出2-3个具体的改进点，按照由简到难的顺序排列
3. 对于每个改进点，给出思路提示，但不直接提供完整代码
4. 添加一个启发性问题，引导学生思考下一步
5. 提供一些调试和测试建议

你的指导应该是温和鼓励的，目的是帮助学生学习思考过程，而不是简单地完成任务。
请注意代码质量、算法效率和编程规范等方面的指导。

请以Markdown格式输出你的指导建议。
"""
            # 调用大模型生成指导
...
def generate_answer_to_question(code, question, assignment_title, assignment_description, language="cpp"):
    ...
    if initialized and guidance_generator:
        try:
            ...
            # 增强提示词，提供更详细的指导方向
            prompt = f"""
作为一名经验丰富的编程教师，我需要回答学生关于代码的问题。
请用专业、耐心且鼓励的语气回答学生的编程问题。
...
请按照以下原则回答：
1. 用友好且鼓励的语气回答，表明编程中遇到困难是正常的
2. 根据问题类型提供不同深度的指导：
   - 概念性问题：解释清楚基本原理
   - 代码错误：帮助诊断可能的错误并提供修复思路
   - 算法问题：解释算法思想并引导学生理解
   - 实现问题：提供思路和方向，但不直接给出完整实现
3. 如果学生问题不清晰，尝试理解其真实需求，引导他们表达更明确的问题
4. 对于代码中的具体问题，精确定位到相关代码段，并详细解释
5. 如需提供代码示例，请标记语言类型，并确保代码片段简洁易懂
6. 对于复杂问题，将解决方案分解为多个步骤
7. 在回答的最后，适当提供延伸学习资源或思考问题

请以Markdown格式回答，使用适当的标题、列表、代码块等元素使回答清晰易读。
请确保回答深入浅出，既照顾初学者理解，又能传授编程的正确思维方式。
"""
